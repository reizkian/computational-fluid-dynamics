...
N_time = 10000  # untuk menghitung 1 detik 
...
P[0,:] = 1000     #Induksi Tekanan pada sebelah kiri
...
Nd = Nr*2
tetha = np.linspace(0,180,Nd)
nx = np.arange(Na-Nr,Na+Nr,1)
ny = np.floor(np.sqrt((Nr)**2-(nx-Na)**2 )+Nb)
nx = nx.astype(int)
ny = ny.astype(int)
P1_tetha = np.zeros([Nd])
P2_tetha = np.zeros([Nd])
P3_tetha = np.zeros([Nd])
# Mulai Iterasi
#------------------------------------------------------------------------------
...
fig10 =  plt.figure() #left profile
plt.plot(tetha,P1_tetha,'r*-', label='t=0.0016 sec')
plt.plot(tetha,P2_tetha,'gx-', label='t=0.003 sec')
plt.plot(tetha,P3_tetha,'bs-', label='t=0.005 sec')
plt.grid()
plt.xlabel('y')
plt.ylabel('P(x)')
plt.title('Tekanan P(x) pada y=L/2')
