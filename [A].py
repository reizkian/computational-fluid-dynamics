...
Mydataset = pd.read_csv('path4.csv') #Import data elevasi
print("* * IMPORTING ELEVATION DATA * *")
print("source : United States Geological Survey")
print("link   : https://earthexplorer.usgs.gov/")
print("------------------------------------------------------------")
Elevation_raw = Mydataset.iloc[:,3].values/1000
Elevation = np.zeros([Nx])
lenElevation = len(Elevation)
object_array = np.zeros([Nx,Ny])
Ngaris_i = int( Nx*(2.6/Lx) )
